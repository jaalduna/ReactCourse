...
export const ScrambleWords = () => {
  ...
  const { totalWords, maxSkips, maxAllowErrors } = params;
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch({ type: "HANDLE_GUESS" });
  };

  const handleSkip = () => {
    dispatch({ type: "HANDLE_SKIP" });
  };

  const handlePlayAgain = () => {
    dispatch({ type: "HANDLE_PLAY_AGAIN", payload: getInitialState() });
  };
  ...
  const scrambledWord = useMemo(() => scrambleWord(currentWord), [currentWord]);
